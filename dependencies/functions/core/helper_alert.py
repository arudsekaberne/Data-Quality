...
class HelperAlert:
    ...
    @staticmethod
    @HelperVault.retry_connection_error(max_retries = 3)
    def send_email_notification(p_job_config: JobConfigModel, p_job_batch_id: str) -> Tuple[JobLogModel, namedtuple]:
        ...
        OutlookAlert.send(
            ...
            subject = f"{'TEST' if EnvUtil.is_dev() else 'LIVE'} [DQ] {notification_info.emoji} {p_job_config.job_name} - {notification_info.status} | Batch ID: {p_job_log_model.batch_id}",
